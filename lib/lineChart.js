...
import {compareValues, formatValue, parameterValue, valueParameter} from './data.js';
...
struct LineChart
    ...
    # The X-axis tick mark
    optional LineChartAxisTicks xtick

    # The Y-axis tick mark
    optional LineChartAxisTicks ytick

    # The X-axis annotations
    optional LineChartAxisAnnotation[len > 0] xline

    # The Y-axis annotations
    optional LineChartAxisAnnotation[len > 0] yline
 ...
 * Render a line chart
 *
 * @param {Object} data - The line chart model
 * @param {Object} lineChart - The line chart model
...
export function lineChartElements(data, lineChart, options = null) {
    const chartFontSize = pixelsPerPoint * (options !== null ? options.fontSize : chartFontDefaultSize);
    ...
    const yAxisTicks = [];
    const yTickCount = ('ytick' in lineChart && 'count' in lineChart.ytick ? lineChart.ytick.count : defaultYAxisTickCount);
    const yTickSkip = ('ytick' in lineChart && 'skip' in lineChart.ytick ? lineChart.ytick.skip + 1 : 1);
    const yTickStart = ('ytick' in lineChart && 'start' in lineChart.ytick ? lineChart.ytick.start : yMin);
    const yTickEnd = ('ytick' in lineChart && 'end' in lineChart.ytick ? lineChart.ytick.end : yMax);
    ...
    const xAxisTicks = [];
    const xTickCount = ('xtick' in lineChart && 'count' in lineChart.xtick ? lineChart.xtick.count : defaultXAxisTickCount);
    const xTickSkip = ('xtick' in lineChart && 'skip' in lineChart.xtick ? lineChart.xtick.skip + 1 : 1);
    const xTickStart = ('xtick' in lineChart && 'start' in lineChart.xtick ? lineChart.xtick.start : xMin);
    const xTickEnd = ('xtick' in lineChart && 'end' in lineChart.xtick ? lineChart.xtick.end : xMax);
