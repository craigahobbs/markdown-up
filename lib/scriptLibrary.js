...
import {addCalculatedField, aggregateData, filterData, joinData, parseCSV, sortData, validateData} from './data.js';
...
export const markdownScriptFunctions = {
    ...
    // $arg fieldName: The name of the field to aggregate
    // $arg func: Optional (default is "average"). The aggregation function. Valid values are "average", "count", "max", "min", and "sum".
    // $arg categories: Optional (default is null). The aggregation category field names.
    // $arg aggregateName: Optional (default is null). The aggregated field name. If null, "fieldName" is used.
    // $return: The aggregated data array
    'dataAggregate': ([data, fieldName, func = 'average', categories = null, aggregateName = null]) => (
        aggregateData(data, fieldName, func, categories, aggregateName)
    ),
    ...
    'documentReset': (unused, options) => {
        const {runtime} = options;
        runtime.isDocumentReset = true;
    },
    ...
    'drawOnClick': ([callback], options) => {
        ...
        clickElement.callback = (element) => {
            element.addEventListener('click', (event) => {
                ...
                options.statementCount = 0;
                callback([event.clientX - boundingRect.left, event.clientY - boundingRect.top], options);
                options.runtimeUpdateFn();
            });
        };
    },
    ...
    'schemaPrint': ([types, typeName, actionURLs = null], options) => {
        const {runtime} = options;
        ...
        runtime.setElements();
        runtime.addElements(schemaMarkdownDoc(types, typeName, schemaOptions));
    },
                ...
                elements.callback = (element) => {
                    ...
                    for (const [elementEvent, elementEventCallback] of Object.entries(elementEvents)) {
                        element.addEventListener(elementEvent, (event) => {
                            ...
                            options.statementCount = 0;
                            elementEventCallback(eventArgs, options);
                            options.runtimeUpdateFn();
                        });
                    }
                };
