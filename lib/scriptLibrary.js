...
export const markdownScriptFunctions = {
    ...
    'setWindowResize': ([callback], options) => {
        ...
        runtime.windowResize = async () => {
            ...
            try {
                ...
                if ('logFn' in options) {
                    options.logFn(`MarkdownUp: Error executing setWindowResize callback: ${message}`);
                }
            }
        ...
        runtime.windowTimeout = [
            async () => {
                ...
                try {
                    ...
                    if ('logFn' in options) {
                        options.logFn(`MarkdownUp: Error executing setWindowTimeout callback: ${message}`);
                    }
                }
                options.runtimeUpdateFn();
            },
    ...
    // $doc: event callback function. Data can be passed to the callback functions by setting a
    // $doc: "callbackData" member.
...
function elementModelWrapCallbacks(elements, options) {
    ...
    if (elementEvents !== null && typeof elementEvents === 'object') {
        // Get the callback data, if any
        let callbackData = null;
        if ('callbackData' in elements) {
            ({callbackData} = elements);
            delete elements.callbackData;
        }
