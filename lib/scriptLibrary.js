...
// $doc: myAppMain()
// $doc: ```
// $arg callback: The keydown event callback function that takes a single
// $arg callback: [key event object](model.html#var.vName='DocumentKeyEvent') argument, `event`.
...
export const documentKeyEventTypes = parseSchemaMarkdown(`\
group "Events"


# A document key event object
struct DocumentKeyEvent
    ...
    # The legacy numeric code (e.g., 65 for 'a')
    string keyCode

    # If true, the control key is pressed
    bool ctrlKey

    # If true, the alt key is pressed
    bool altKey

    # If true, the shift key is pressed
    bool shiftKey

    # If true, the cmd key is pressed
    bool metaKey

    # If true, the key is held down
    bool repeat

    # 0=standard, 1=left, 2=right
    int location
`);
...
function schemaElements(args, options) {
    const [types, typeName, actionURLs = null, actionCustom] = valueArgsValidate(schemaElementsArgs, args);
    const schemaOptions = {
    ...
    if (actionCustom) {
        schemaOptions.actionCustom = true;
    }
...
const schemaElementsArgs = valueArgsModel([
    ...
    {'name': 'actionCustom', 'type': 'boolean', 'default': false}
]);
