...
function valueArgsValidate(args, fnArgs) {
    const argsLength = args.length;
    const fnArgsArgs = fnArgs.args;
    const fnArgsLength = fnArgsArgs.length;
    for (let ix = 0; ix < fnArgsLength; ix++) {
        const fnArg = fnArgsArgs[ix];
        if (ix < argsLength) {
            const argValue = args[ix];
            const argType = fnArg.type;
            if (argType === 'boolean') {
                args[ix] = valueBoolean(argValue);
            } else if ((argValue === null && !fnArg.nullable) ||
                (argValue !== null &&
                 (argType === 'array' && !Array.isArray(argValue)) ||
                 (argType === 'number' && typeof argValue !== 'number') ||
                 (argType === 'object' && !(typeof argValue === 'object' && Object.getPrototypeOf(argValue) === Object.prototype)) ||
                 (argType === 'string' && typeof argValue !== 'string'))
               ) {
                throw Error(
                    `Function "${fnArgs.name}" called with invalid "${fnArg.name}" argument value, ${valueString(argValue)}`
                );
            }
        } else {
            args.push(fnArg.default ?? null);
        }
    }
    return args;
}
...
function dataLineChart(args, options) {
    const [data, lineChart] = valueArgsValidate(args, dataLineChartArgs);
...
const dataTableArgs = valueArgsModel({
    ...
    'args': [
        {'name': 'data', 'type': 'array'},
        {'name': 'dataTable', 'type': 'object', 'nullable': true, 'default': null}
    ]
});
function dataTable(args, options) {
    const [data, dataTableModel] = valueArgsValidate(args, dataTableArgs);
    const {runtime} = options;
    runtime.setElements();
...
const documentInputValueArgs = valueArgsModel({
    'name': 'documentInputValue',
    'args': [
        {'name': 'id', 'type': 'string'}
    ]
});
function documentInputValue(args, options) {
    const [id] = valueArgsValidate(args, documentInputValueArgs);
...
const documentSetFocusArgs = valueArgsModel({
    'name': 'documentSetFocus',
    'args': [
        {'name': 'id', 'type': 'string'}
    ]
});
function documentSetFocus(args, options) {
    const [id] = valueArgsValidate(args, documentSetFocusArgs);
    const {runtime} = options;
...
const documentSetResetArgs = valueArgsModel({
    'name': 'documentSetReset',
    'args': [
        {'name': 'id', 'type': 'string'}
    ]
});
function documentSetReset(args, options) {
    const [id] = valueArgsValidate(args, documentSetResetArgs);
    const {runtime} = options;
...
function documentSetTitle(args, options) {
    const [title] = valueArgsValidate(args, documentSetTitleArgs);
    const {runtime} = options;
...
function documentURL(args, options) {
    const [url] = valueArgsValidate(args, documentURLArgs);
...
function drawArc(args, options) {
    const [rx, ry, angle, largeArcFlag, sweepFlag, x, y] = valueArgsValidate(args, drawArcArgs);
    const {runtime} = options;
...
function drawClose(unused, options) {
    const {runtime} = options;
    runtime.setDrawingPath();
...
function drawEllipse([cx, cy, rx, ry], options) {
    const {runtime} = options;
    const svg = runtime.setDrawing();
    svg.elem.push({
        'svg': 'ellipse',
...
function drawHeight(unused, options) {
...
function drawImage([x, y, width, height, href], options) {
    const {runtime} = options;
    const svg = runtime.setDrawing();
    svg.elem.push({
        'svg': 'image',
...
function drawNew([width, height], options) {
    const {runtime} = options;
...
function drawOnClick([callback], options) {
    const {runtime} = options;
    const svg = runtime.setDrawing();
...
function drawPathRect(args, options) {
    const [x, y, width, height] = valueArgsValidate(args, drawPathRectArgs);
    const {runtime} = options;
    runtime.setDrawingPath();
    runtime.drawingPath.push(`M ${x.toFixed(svgPrecision)} ${y.toFixed(svgPrecision)}`);
...
function drawRect([x, y, width, height, rx = null, ry = null], options) {
    const {runtime} = options;
    const svg = runtime.setDrawing();
    const element = {
        'svg': 'rect',
...
function drawStyle(args, options) {
    const [stroke, strokeWidth, fill, strokeDashArray] = valueArgsValidate(args, drawStyleArgs);
    const {runtime} = options;
...
function drawText([text, x, y, textAnchor = 'middle', dominantBaseline = 'middle'], options) {
    const {runtime} = options;
    const svg = runtime.setDrawing();
    svg.elem.push({
        'svg': 'text',
...
function drawTextHeight([text, width], options) {
...
function drawTextStyle([fontSizePx = null, textFill = 'black', bold = false, italic = false, fontFamily = null], options) {
    const {runtime} = options;
...
function drawWidth(unused, options) {
...
function elementModelRender([elements], options) {
    const {runtime} = options;
    runtime.setElements();
...
function localStorageClear(unused, options) {
...
function localStorageGet([key], options) {
...
// $arg key: The key string
function localStorageRemove([key], options) {
...
function localStorageSet([key, value], options) {
...
function markdownTitle([markdownModel]) {
...
function schemaElements([types, typeName, actionURLs = null], options) {
...
function sessionStorageClear(unused, options) {
...
function sessionStorageGet([key], options) {
...
// $arg key: The key string
// $arg value: The value string
function sessionStorageSet([key, value], options) {
...
// $arg key: The key string
function sessionStorageRemove([key], options) {
...
function windowHeight(unused, options) {
...
function windowSetLocation([location], options) {
    const {runtime} = options;
...
function windowSetResize([callback], options) {
    const {runtime} = options;
...
function windowSetTimeout([callback, delay], options) {
    const {runtime} = options;
...
function windowWidth(unused, options) {
