...
function dataTable([data, dataTableModel = null], options) {
    const {runtime} = options;
    runtime.setElements();
...
function documentFontSize(unused, options) {
...
function documentInputValue([id], options) {
...
function documentSetFocus([id], options) {
    const {runtime} = options;
...
// $arg id: The element ID
function documentSetReset([id], options) {
    const {runtime} = options;
...
function documentSetTitle([title], options) {
    const {runtime} = options;
...
function documentURL([url], options) {
...
function drawArc([rx, ry, angle, largeArcFlag, sweepFlag, x, y], options) {
    const {runtime} = options;
...
function drawPathRect([x, y, width, height], options) {
    const {runtime} = options;
    runtime.setDrawingPath();
    runtime.drawingPath.push(`M ${x.toFixed(svgPrecision)} ${y.toFixed(svgPrecision)}`);
...
function drawStyle([stroke = 'black', strokeWidth = 1, fill = 'none', strokeDashArray = 'none'], options) {
    const {runtime} = options;
