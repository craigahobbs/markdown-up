...
import {valueBoolean, valueString} from 'bare-script/lib/value.js';
...
// Function argument validation helper
function valueArgsValidate(fnName, fnArgs, args) {
...
function valueArgsError(fnName, argName, argValue) {
    throw Error(`Function "${fnName}" called with invalid "${argName}" argument value, ${valueString(argValue)}`);
}


// Helper to validate function arguments models
function valueArgsModel(fnArgs) {
...
function dataLineChart(args, options) {
    const [data, lineChart] = valueArgsValidate('dataLineChart', dataLineChartArgs, args);
...
function dataTable(args, options) {
    const [data, dataTableModel] = valueArgsValidate('dataTable', dataTableArgs, args);
    const {runtime} = options;
    runtime.setElements();
...
function documentInputValue(args, options) {
    const [id] = valueArgsValidate('documentInputValue', documentInputValueArgs, args);
...
function documentSetFocus(args, options) {
    const [id] = valueArgsValidate('documentSetFocus', documentSetFocusArgs, args);
    const {runtime} = options;
...
function documentSetReset(args, options) {
    const [id] = valueArgsValidate('documentSetReset', documentSetResetArgs, args);
    const {runtime} = options;
...
function documentSetTitle(args, options) {
    const [title] = valueArgsValidate('documentSetTitle', documentSetTitleArgs, args);
    const {runtime} = options;
...
function documentURL(args, options) {
    const [url] = valueArgsValidate('documentURL', documentURLArgs, args);
...
function drawArc(args, options) {
    const [rx, ry, angle, largeArcFlag, sweepFlag, x, y] = valueArgsValidate('drawArc', drawArcArgs, args);
    const {runtime} = options;
...
function drawCircle(args, options) {
    const [cx, cy, r] = valueArgsValidate('drawCircle', drawCircleArgs, args);
    const {runtime} = options;
...
function drawEllipse(args, options) {
    const [cx, cy, rx, ry] = valueArgsValidate('drawEllipse', drawEllipseArgs, args);
    const {runtime} = options;
    const svg = runtime.setDrawing();
    svg.elem.push({
        'svg': 'ellipse',
...
function drawHLine(args, options) {
    const [x] = valueArgsValidate('drawHLine', drawHLineArgs, args);
    const {runtime} = options;
    runtime.setDrawingPath();
...
function drawImage(args, options) {
    const [x, y, width, height, href] = valueArgsValidate('drawImage', drawImageArgs, args);
    const {runtime} = options;
    const svg = runtime.setDrawing();
    svg.elem.push({
        'svg': 'image',
...
function drawLine(args, options) {
    const [x, y] = valueArgsValidate('drawLine', drawLineArgs, args);
    const {runtime} = options;
    runtime.setDrawingPath();
...
function drawMove(args, options) {
    const [x, y] = valueArgsValidate('drawMove', drawMoveArgs, args);
    const {runtime} = options;
    runtime.setDrawingPath();
...
function drawNew(args, options) {
    const [width, height] = valueArgsValidate('drawNew', drawNewArgs, args);
    const {runtime} = options;
...
function drawOnClick(args, options) {
    const [callback] = valueArgsValidate('drawOnClick', drawOnClickArgs, args);
    const {runtime} = options;
    const svg = runtime.setDrawing();
...
function drawPathRect(args, options) {
    const [x, y, width, height] = valueArgsValidate('drawPathRect', drawPathRectArgs, args);
    const {runtime} = options;
    runtime.setDrawingPath();
    runtime.drawingPath.push(`M ${x.toFixed(svgPrecision)} ${y.toFixed(svgPrecision)}`);
...
const drawRectArgs = valueArgsModel([
    ...
    {'name': 'width', 'type': 'number'},
    {'name': 'height', 'type': 'number'},
    {'name': 'rx', 'type': 'number', 'default': null},
    {'name': 'ry', 'type': 'number', 'default': null}
]);
function drawRect(args, options) {
    const [x, y, width, height, rx = null, ry = null] = valueArgsValidate('drawRect', drawRectArgs, args);
    const {runtime} = options;
    const svg = runtime.setDrawing();
    const element = {
        'svg': 'rect',
...
function drawStyle(args, options) {
    const [stroke, strokeWidth, fill, strokeDashArray] = valueArgsValidate('drawStyle', drawStyleArgs, args);
    const {runtime} = options;
...
function drawText(args, options) {
    const [text, x, y, textAnchor, dominantBaseline] = valueArgsValidate('drawText', drawTextArgs, args);
    const {runtime} = options;
    const svg = runtime.setDrawing();
    svg.elem.push({
        'svg': 'text',
...
function drawTextHeight(args, options) {
    const [text, width] = valueArgsValidate('drawTextHeight', drawTextHeightArgs, args);
...
function drawTextStyle(args, options) {
    const [fontSizePx, textFill, bold, italic, fontFamily] = valueArgsValidate('drawTextStyle', drawTextStyleArgs, args);
    const {runtime} = options;
...
function drawTextWidth(args) {
    const [text, fontSizePx] = valueArgsValidate('drawTextWidth', drawTextWidthArgs, args);
...
function drawVLine(args, options) {
    const [y] = valueArgsValidate('drawVLine', drawVLineArgs, args);
    const {runtime} = options;
    runtime.setDrawingPath();
...
function localStorageGet(args, options) {
    const [key] = valueArgsValidate('localStorageGet', localStorageGetArgs, args);
...
function localStorageRemove(args, options) {
    const [key] = valueArgsValidate('localStorageRemove', localStorageRemoveArgs, args);
...
function localStorageSet(args, options) {
    const [key, value] = valueArgsValidate('localStorageSet', localStorageSetArgs, args);
...
function markdownEscape(args) {
    const [text] = valueArgsValidate('markdownEscape', markdownEscapeArgs, args);
...
function markdownHeaderId(args) {
    const [text] = valueArgsValidate('markdownHeaderId', markdownHeaderIdArgs, args);
...
function markdownTitle(args) {
    const [markdownModel] = valueArgsValidate('markdownTitle', markdownTitleArgs, args);
...
function schemaElements(args, options) {
    const [types, typeName, actionURLs = null] = valueArgsValidate('schemaElements', schemaElementsArgs, args);
...
function sessionStorageGet(args, options) {
    const [key] = valueArgsValidate('sessionStorageGet', sessionStorageGetArgs, args);
...
function sessionStorageRemove(args, options) {
    const [key] = valueArgsValidate('sessionStorageRemove', sessionStorageRemoveArgs, args);
...
function sessionStorageSet(args, options) {
    const [key, value] = valueArgsValidate('sessionStorageSet', sessionStorageSetArgs, args);
...
function windowSetLocation(args, options) {
    const [location] = valueArgsValidate('windowSetLocation', windowSetLocationArgs, args);
    const {runtime} = options;
...
function windowSetResize(args, options) {
    const [callback] = valueArgsValidate('windowSetResize', windowSetResizeArgs, args);
    const {runtime} = options;
...
function windowSetTimeout(args, options) {
    const [callback, delay] = valueArgsValidate('windowSetTimeout', windowSetTimeoutArgs, args);
    const {runtime} = options;
