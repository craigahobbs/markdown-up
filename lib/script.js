...
import {lintScript} from '../calc-script/lib/model.js';
 ...
 * @property {Object} params - The [hash parameters object]{@link https://craigahobbs.github.io/markdown-up/#cmd.help=1}
    ...
    try {
        ...
        if ('logFn' in options) {
            ...
            if (warnings.length === 0) {
                options.logFn(`MarkdownUp: Script static analysis... OK`);
            } else {
                options.logFn(`MarkdownUp: Script static analysis ... ${warnings.length} warning${warnings.length > 1 ? 's' : ''}:`);
 ...
 * @property {?function} windowResize - The the runtime-set resize callback
 * @property {?Array} windowTimeout - The the runtime-set timeout args (callback, delay)
 *
 * @ignore
 */
