...
export async function markdownScriptCodeBlock(codeBlock, options) {
    ...
    try {
        ...
        if ('logFn' in options && options.debug) {
            const warnings = lintScript(script, options.globals);
    ...
    finishMarkdown() {
        if (this.markdown !== null) {
            const markdownElements =
                  (markdown, markdownOptions) => this.options.globals.markdownElements([markdown, markdownOptions], this.options);
            const parseMarkdown = (text) => this.options.globals.markdownParse([text], this.options);
            const contentElements = markdownElements(parseMarkdown(this.markdown), this.options.markdownOptions);
