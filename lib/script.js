...
import {barescriptLintScript, barescriptParseScript} from '../bare-script/lib/runtime.js';
import {markdownElements, markdownParse} from '../bare-script/lib/include.js';
import {elementModelWrapCallbacks} from './scriptLibrary.js';
...
export async function markdownScriptCodeBlock(codeBlock, options) {
    ...
    try {
        // Parse the script
        const script = barescriptParseScript(codeBlock.lines, codeBlock.startLineNumber + 1);
        ...
        if ('logFn' in options && options.debug) {
            const warnings = barescriptLintScript(script, options.globals);
...
export class MarkdownScriptRuntime {
    ...
    finishMarkdown() {
        if (this.markdown !== null) {
            const contentElements = markdownElements(markdownParse(this.markdown), this.options.markdownOptions);
            elementModelWrapCallbacks(contentElements, this.options);
            this.addElements(contentElements);
            this.markdown = null;
        }
    }
}
