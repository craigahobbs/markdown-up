 ...
 * @param {module:lib/util~MarkdownScriptOptions} options - Markdown script options
 ...
 * @property {module:lib/util~MarkdownScriptOptions} options - Markdown script options
...
export class MarkdownScriptRuntime {
    ...
    createGlobals() {
        return {
            ...
            'setDocumentTitle': ([title]) => this.documentSetTitle(title),
            ...
            'localStorageGetItem': ([key]) => this.options.window.localStorage.getItem(key),
            'localStorageSetItem': ([key, value]) => this.options.window.localStorage.setItem(key, value),
            'localStorageRemoveItem': ([key]) => this.options.window.localStorage.removeItem(key),
            ...
            'sessionStorageGetItem': ([key]) => this.options.window.sessionStorage.getItem(key),
            'sessionStorageSetItem': ([key, value]) => this.options.window.sessionStorage.setItem(key, value),
            'sessionStorageRemoveItem': ([key]) => this.options.window.sessionStorage.removeItem(key),
    ...
    setDrawing(newDrawing = false) {
        this.setElements();
        let svgElement = (this.elements.length !== 0 ? this.elements[this.elements.length - 1] : null);
        const isDrawing = (svgElement !== null && svgElement.svg === 'svg');
        if (!isDrawing || newDrawing) {
            svgElement = {
                'svg': 'svg',
                'attr': {
    ...
    finishDrawingPath() {
        if (this.drawingPath !== null) {
            const svg = this.elements[this.elements.length - 1];
