...
export class MarkdownUp {
    ...
    async render(forceRender = false) {
        ...
        // URL hash ID?
        const hashId = getHashID(this.window.location.hash);
        if (!isError && hashId !== null) {
            // Special "top" hash ID?
            if (hashId === 'top' && this.window.document.getElementById(this.window.location.hash.slice(1)) === null) {
                this.window.scrollTo(0, 0);
            } else {
                // Re-navigate to scroll to the hash ID since it was just rendered. The re-render is
                // short-circuited by the unchanged hash param check above.
