...
export class MarkdownUp {
        ...
        if ('var' in this.params) {
            ...
            for (const varName of Object.keys(this.params.var)) {
                ...
                try {
                    ...
                    if (logFn !== null) {
                        logFn(`Error evaluating variable "${varName}" expression "${varExprStr}": ${message}`);
                    }
                }
            }
        }
        ...
        return !this.menu ? null : [
            {
                ...
                'elem': this.menuValueToggle('menu', menuSVGElements, 'Menu')
            },
            ...
            !('menu' in this.paramsSession) ? null : {
                ...
                'attr': {'class': 'menu'},
                'elem': [
                    this.menuViewToggle('markdown', markdownSVGElements, 'Show Markdown'),
                    this.menuValueCycle('fontSize', fontSizeSVGElements, 'Cycle font size', this.fontSize, 2),
                    this.menuValueCycle('lineHeight', lineHeightSVGElements, 'Cycle line height', this.lineHeight, 0.2),
                    this.menuValueToggle('debug', debugSVGElements, 'Debug'),
                    this.menuViewToggle('help', helpSVGElements, 'Help')
                ]
            }
        ];
    }
    ...
    menuValueToggle(value, svgElements, ariaLabel) {
        return {
            'html': 'a',
            'attr': {'style': 'cursor: pointer; user-select: none;', 'aria-label': ariaLabel},
            'elem': svgElements(value in this.paramsSession),
            'callback': (element) => {
                element.addEventListener('click', () => {
                    if (value in this.paramsSession) {
                        delete this.paramsSession[value];
                    } else {
                        this.paramsSession[value] = 1;
                    }
    ...
    menuViewToggle(value, svgElements, ariaLabel) {
        return {
            'html': 'a',
            'attr': {'style': 'cursor: pointer; user-select: none;', 'aria-label': ariaLabel},
            'elem': svgElements(this.paramsSession.view === value),
            'callback': (element) => {
                element.addEventListener('click', () => {
                    if (this.paramsSession.view === value) {
                        delete this.paramsSession.view;
                    } else {
                        this.paramsSession.view = value;
                    }
                    this.window.sessionStorage.setItem('MarkdownUp', JSON.stringify(this.paramsSession));
                    this.render(true);
                });
    ...
    menuValueCycle(valueName, svgElements, ariaLabel, valueDefault, valueDelta) {
        return {
            'html': 'a',
            'attr': {'style': 'cursor: pointer; user-select: none;', 'aria-label': ariaLabel},
            'elem': svgElements(),
...
// Menu icons
const menuSVGElements = () => menuSVGHelper('M3,5 L21,5 M3,12 L21,12 M3,19 L21,19', false, 3, null, 32);
const debugSVGElements = (isChecked) => menuSVGHelper(
    'M12,5 A4,7,0,1,0,12,19 A4,7,0,1,0,12,5 M9,9 L15,9 M9,9 L4,6 M9,12 L3,12 M9,15 L4,18 M15,9 L20,6 M15,12 L21,12 M15,15 L20,18',
    isChecked
);
const fontSizeSVGElements = () => menuSVGHelper('M4,22 L10,2 L14,2 L20,22 M6,14 L18,14', false, 4);
const helpSVGElements = (isChecked) => menuSVGHelper('M7,9 L7,4 L17,4 L17,12 L12,12 L12,16 M12,19 L12,22', isChecked);
const lineHeightSVGElements = () => menuSVGHelper('M2,3 L22,3 M2,9 L22,9 M2,15 L22,15 M2,21 L22,21', false, 2);
const markdownSVGElements = (isChecked) => (
    menuSVGHelper('M4,2 L20,2 L20,22 L4,22 Z', isChecked, 3, 'M7,7.5 L17,7.5 M7,12 L17,12 M7,16.5 L17,16.5')
);


// Menu SVG helper function
function menuSVGHelper(pathD, isChecked = false, strokeWidth = 3, pathD2 = null, iconSize = 48) {
    const borderSize = (0.125 * iconSize).toFixed(3);
    const innerSize = (0.75 * iconSize).toFixed(3);
    const iconElems = [
        {
            'svg': 'path',
            'attr': {'fill': 'none', 'stroke': (isChecked ? 'white' : 'black'), 'stroke-width': strokeWidth, 'd': pathD}
        }
    ];
    const svg = {
        'svg': 'svg',
        'attr': {'width': iconSize, 'height': iconSize},
        'elem': [
            {
                'svg': 'g',
    ...
    if (isChecked) {
        svg.elem.unshift({
            'svg': 'rect',
            'attr': {'fill': 'black', 'stroke': 'none', 'width': iconSize, 'height': iconSize}
        });
    }
    if (pathD2 !== null) {
        iconElems.push({
            'svg': 'path',
            'attr': {'fill': 'none', 'stroke': (isChecked ? 'white' : 'black'), 'stroke-width': strokeWidth - 1, 'd': pathD2}
        });
    }
    return svg;
}
