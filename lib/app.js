...
export class MarkdownUp {
    ...
    async render(forceRender = false) {
        ...
        // If the window location hash exists in the document, re-navigate there since it was just rendered.
        // After the first render, re-render is short-circuited by the unchanged hash param check above.
        if (!isError && this.window.document.getElementById(this.window.location.hash.slice(1)) !== null) {
