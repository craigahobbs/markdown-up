...
export class MarkdownUp {
    ...
    async render(forceRender = false) {
        ...
        try {
            ...
            const paramsJSON = jsonStringifySortKeys(this.params);
            if (!forceRender && paramsPrev !== null && jsonStringifySortKeys(paramsPrev) === paramsJSON) {
                this.window.console.log(`MarkdownUp: Rendering skipped ${paramsJSON}`);
                return;
            }
            this.window.console.log(`MarkdownUp: Rendering ${paramsJSON}`);
        } catch ({message}) {
            result = {
                'title': 'MarkdownUp',
        ...
        // If there is a URL hash ID, re-navigate to go there since it was just rendered.
