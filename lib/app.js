...
export class MarkdownUp extends ElementApplication {
    ...
    async main() {
        // Help?
        if ('cmd' in this.params && 'help' in this.params.cmd && this.params.cmd.help === 'Help') {
            return {
                'elements': new UserTypeElements(this.params).getElements(this.hashTypes, this.hashType)
            };
        } else if ('cmd' in this.params && 'help' in this.params.cmd) {
            return {
                'elements': new UserTypeElements(this.params).getElements(chartModel.types, `${this.params.cmd.help}Chart`)
            };
        }
