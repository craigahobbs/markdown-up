...
#
# [MarkdownUp](https://craigahobbs.github.io/markdown-up/) is a Markdown viewer application.
#
# ## Hash Parameters
#
...
export class MarkdownUp {
     ...
     * @property {Object} [options] - The [application options]{@link module:lib/app~MarkdownUpOptions}
     */
    ...
    async render() {
        ...
        if (this.runtimeWindowResize !== null) {
        ...
        // Focus?
        if ('focus' in result) {
            const element = this.window.document.getElementById(result.focus);
            if (element !== null) {
                element.focus();
    ...
    createScriptOptions(resourceURL) {
        ...
        const scriptOptions = {
            ...
            'fetchFn': async (fetchURL, init) => this.window.fetch(fetchURL, init),
        ...
        scriptOptions.runtimeUpdateFn = () => {
            ...
            // Reset the runtime
            const {isDocumentReset} = runtime;
            const {windowLocation} = runtime;
            const documentElements = runtime.reset();

            // Render the elment model
            renderElements(
            ...
            if (runtime.documentFocus !== null) {
                const element = this.window.document.getElementById(runtime.documentFocus);
                if (element !== null) {
                    element.focus();
                    if ('selectionStart' in element) {
                        element.selectionStart = element.value.length;
