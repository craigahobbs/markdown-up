 ...
 * @property {number} [runtimeTimeoutId = null] - The runtime timeout ID
 */
export class MarkdownUp extends ElementApplication {
    ...
    async main() {
        ...
        if (runtime.setNavigateTimeoutURL !== null && runtime.setNavigateTimeoutDelay !== null) {
            let navigateURL = runtime.setNavigateTimeoutURL;
            if (navigateURL.startsWith('#')) {
                navigateURL = hashFn(navigateURL);
            }
            const navigateFunc = () => {
                this.window.location.href = navigateURL;
            };
            this.runtimeTimeoutId = this.window.setTimeout(navigateFunc, runtime.setNavigateTimeoutDelay);
        }
