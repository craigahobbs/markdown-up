...
export function joinData(leftData, rightData, joinExpr, rightExpr = null, isLeftJoin = false, variables = null, options = null) {
    ...
    const evalOptions = options ?? (variables !== null ? {'globals': variables} : null);
    ...
    for (const rightRow of rightData) {
        const evalVariables = (variables === null ? rightRow : {...rightRow, ...variables});
        const categoryKey = jsonStringifySortKeys(evaluateExpression(rightExpression, evalOptions, evalVariables));
    ...
    for (const leftRow of leftData) {
        const evalVariables = (variables === null ? leftRow : {...leftRow, ...variables});
        const categoryKey = jsonStringifySortKeys(evaluateExpression(leftExpression, evalOptions, evalVariables));
        if (categoryKey in rightCategoryRows) {
...
export function addCalculatedField(data, fieldName, expr, variables = null, options = null) {
    ...
    const evalOptions = options ?? (variables !== null ? {'globals': variables} : null);
    for (const row of data) {
        const evalVariables = (variables === null ? row : {...row, ...variables});
        row[fieldName] = evaluateExpression(calcExpr, evalOptions, evalVariables);
    }
    return data;
}
...
export function filterData(data, expr, variables = null, options = null) {
    ...
    for (const row of data) {
        const evalVariables = (variables === null ? row : {...row, ...variables});
        if (evaluateExpression(filterExpr, evalOptions, evalVariables)) {
            result.push(row);
        }
    }
