...
test('script library, elementModelRender callback SVG click', async () => {
    const runtime = testRuntime();
    let runtimeUpdateCount = 0;
    runtime.options.runtimeUpdateFn = () => ++runtimeUpdateCount;
    ...
    assert.deepEqual(eventHandlerArgs, []);
    assert.equal(runtimeUpdateCount, 0);
    ...
    await mockElementEvents.click(mockEvent);
    assert.deepEqual(eventHandlerArgs, [[200, 100, 800, 600]]);
    assert.equal(runtimeUpdateCount, 1);

    // Call the element handler function as top-level SVG
    const mockEventTop = {
        'clientX': 300,
        'clientY': 150,
        'target': {
    ...
    eventHandlerArgs.length = 0;
    await mockElementEvents.click(mockEventTop);
    assert.deepEqual(eventHandlerArgs, [[200, 100, 800, 600]]);
    assert.equal(runtimeUpdateCount, 2);
});
